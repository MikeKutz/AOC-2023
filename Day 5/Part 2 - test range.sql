...
56 93 4
  ...
  order by step#, source_start
), seeds (seed_rn, seed#, seed_end) as (
select seed_rn, seed_start, seed_start + seed_range -1 seed_end
from (
  select seed_rn - abs(1- mod(seed_rn, 2)) seed_rn
        ,case mod(seed_rn, 2)
          when 1 then 'start#'
          when 0 then 'range'
        end data_type
  ...
  min(seed#)
  for data_type in ( 'start#' as seed_start, 'range' as seed_range)
)
...
select min(dest_start) answer_p2
from map_traversal
where this_step = 7
;
