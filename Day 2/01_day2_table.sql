create table e2023_2 as
with input_data (txt) as ( select
q'[Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
Game 100: 6 green, 15 red, 12 blue; 9 red; 16 red; 17 red, 3 blue, 7 green
  ...
  select substr(s.column_value, 1, instr(s.column_value, ':') -1 ) game_id_str
          ,to_number(substr( substr(s.column_value, 1, instr(s.column_value, ':') -1 ), instr( s.column_value, ' ' ) ) )game#
          ,r.rn
        ,substr(s.column_value, instr(s.column_value, ':') +2 ) game_results
--        ,trim(r.column_value) set_data
--        ,trim(c.column_value) cube_set
        ,trim(substr( trim(c.column_value), instr(trim(c.column_value), ' ' ) +1 )) color_name
        ,to_number(substr( trim(c.column_value), 1, instr(trim(c.column_value), ' ' ) -1 )) color_count
  from input_data i
        cross apply apex_string.split( i.txt, chr(10) ) s
        cross apply (
            select rownum rn, column_value
            from apex_string.split( substr(s.column_value, instr(s.column_value, ':') +2 ), ';' ) 
        ) r
        cross apply apex_string.split( trim(r.column_value), ',' ) c
), game_set_data as (
  select game_id_str, game#
        ,nvl(red,0)   as red
        ,nvl(blue,0)  as blue
        ,nvl(green,0) as green
        ,nvl(red_max,0)   as red_max
        ,nvl(blue_max,0)  as blue_max
        ,nvl(green_max,0) as green_max
  from translated_data
  pivot (
    sum(color_count)
    ,max(color_count) as MAX
  
   for color_name in ('red' as red, 'blue' as blue, 'green' as green )
  )
)
select * from game_set_data;