drop table if exists d19_objects purge;
drop table if exists d19_decisions purge;

create table d19_decisions as
--create table d19_objects as 
with
  input_data (txt) as (
...
{x=2127,m=1623,a=2188,s=1013}
    ...
    from (select rownum step_entry
            ,substr( a.dec_line, 1, instr( a.dec_line, '{' ) - 1) step_name
            ,rtrim( substr( a.dec_line, instr( a.dec_line, '{' ) + 1), '}' ) all_dec
          from decision_tree_unprocessed a) oa
      cross apply (
        select rownum decision_order, trim(column_Value) column_value
        from apex_string.split( oa.all_dec, ',' ) 
...
-- DECISION_ORDER not generated correctly,
-- will materialize into table 1st
select c.*
  ,case
    when dec_symbol = '<' and lhs < comp_value then c.next_step
    when dec_symbol = '=' and lhs = comp_value then c.next_step
    when dec_symbol = '>' and lhs > comp_value then c.next_step
  end decision_made
from (
